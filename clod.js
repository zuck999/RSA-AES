...
class AES {
  ...
  encrypt(message, key) {
    ...
    const paddedMessage = Buffer.concat([messageBytes, Buffer.alloc(paddingLength, paddingLength)]);
    
    const encrypted = [];
    for (let i = 0; i < paddedMessage.length; i += 16) {
      const block = Array.from(paddedMessage.slice(i, i + 16));
...
console.log('AES-RSA HYBRID ENCRYPTION SYSTEM');
console.log('Bachelor Project - Manual Implementation');
console.log('='.repeat(60));
